...
function normalizeDecision(raw: Partial<Decision> & { id?: string; title?: string; question?: string }): Decision {
  return {
    ...
    requiredContextLevel: raw.requiredContextLevel ?? 'squad',
    authorityScope: Array.isArray(raw.authorityScope) ? raw.authorityScope : ['squad'],
    ...
    escalationPath: Array.isArray(raw.escalationPath) ? raw.escalationPath : ['squad'],
...
export default function ProductOwnerView({
  ...
  const inbox = useMemo(
    () => decisions.filter((item) => item.status === 'pending' && item.requiredContextLevel === 'squad'),
    [decisions]
  )
  ...
  const resolve = useCallback(async (decisionId: string, status: 'approved' | 'rejected') => {
    ...
    setError('')
    try {
      const res = await call('decision.resolve', { decisionId, status, actor: 'product-lead' })
  ...
  const escalate = useCallback(async (decision: Decision) => {
    ...
    setError('')
    try {
      const escalationPath = Array.from(new Set([...decision.escalationPath, 'squad', 'tribe']))
      const record = await call('decision.record', {
        initiativeId: decision.initiativeId,
        title: `${decision.title} · Tribe escalation`,
        question: decision.question,
        rationale: decision.rationale || decision.escalationReason || 'Product owner escalation required.',
        requiredContextLevel: 'tribe',
        authorityScope: ['tribe'],
        decisionPolicy: 'delegated_approval',
        inferable: false,
        confidence: Math.min(0.69, decision.confidence || 0.69),
        escalationReason: 'Squad context insufficient or outside product owner delegated authority.',
        escalationPath,
        actor: 'product-lead',
      })
      if (!record.ok) throw new Error(record.error || 'decision_escalation_record_failed')
      const close = await call('decision.resolve', { decisionId: decision.id, status: 'rejected', actor: 'product-lead' })
    ...
    <div className="h-full overflow-y-auto bg-[#F7F6F1] p-4 text-[13px] text-text">
      <div className="mb-4 rounded border border-border bg-white p-3">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-wide text-text-muted">Product Owner View</div>
            <div className="text-[12px] text-text-secondary">Squad-level decision inbox. Resolve when within authority, escalate to tribe when needed.</div>
          </div>
      ...
      <div className="rounded border border-border bg-white p-3">
        <div className="mb-2 text-[11px] uppercase tracking-wide text-text-muted">Pending Product Decisions</div>
        {inbox.length === 0 ? (
          <div className="text-[12px] text-text-muted">No pending product-owner decisions.</div>
        ) : (
          inbox.map((item) => (
            <div key={item.id} className="mb-2 rounded border border-border/70 bg-[#FCFCFA] p-2 text-[12px] last:mb-0">
              ...
              <div className="mt-1 text-[11px] text-text-muted">Path: {(item.escalationPath ?? []).join(' -> ') || 'squad'}</div>
