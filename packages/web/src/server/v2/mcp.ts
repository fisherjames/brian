...
function mapStage(method: string): V2Stage {
  ...
  if (method.startsWith('proposal.generate')) return 'proposal'
...
function upsertProposalRecord(
  ...
  const id = `proposal-${payload.initiativeId}`
  const rel = path.join('brian', 'proposals', `${id}.md`)
  const fullPath = path.join(brainPath, rel)
  const createdAt = fs.existsSync(fullPath)
    ? String(parseFrontmatter(fs.readFileSync(fullPath, 'utf8')).created_at || new Date().toISOString())
  ...
  writeRecordMarkdown(
    ...
    {
      id,
      title: `Director Proposal: ${payload.initiativeTitle}`,
      initiative_id: payload.initiativeId,
      discussion_id: payload.discussionId,
      decision_question: payload.decisionQuestion,
      recommendation: payload.recommendation,
      summary: payload.summary,
      constraints_json: JSON.stringify(payload.constraints),
      options_json: JSON.stringify(payload.options),
      risks_json: JSON.stringify(payload.risks),
      discussion_evidence_json: JSON.stringify(payload.evidence),
      generated_by: payload.actor,
      created_at: createdAt,
      updated_at: new Date().toISOString(),
    },
    [
      `# Director Proposal: ${payload.initiativeTitle}`,
      '',
      '## Initiative Goal + Constraints',
      `- Goal: ${payload.summary || `Deliver ${payload.initiativeTitle} with measurable user impact.`}`,
      ...payload.constraints.map((item) => `- Constraint: ${item}`),
      '',
      ...
      `- Source discussion: [[brian/discussions/${payload.discussionId}]]`,
      ...payload.evidence.map((item) => `- ${item}`),
      '',
      '## Director Recommendation and Decision Question',
      `- Recommendation: ${payload.recommendation}`,
      `- Decision question: ${payload.decisionQuestion}`,
      '',
      '## Outcome Log',
      `- ${new Date().toISOString()} · generated by ${payload.actor}`,
    ].join('\n')
  )
  return {
    id,
...
export function runV2McpCall(brainId: string, method: string, params: Record<string, unknown>): any {
  ...
  if (method === 'proposal.generate') {
    ...
    const explicitDiscussionId = typeof params.discussionId === 'string' ? params.discussionId.trim() : ''
    const discussion =
      models.discussions.find((item) => item.id === explicitDiscussionId)
      ?? models.discussions.find((item) => item.initiativeId === initiativeId && item.layer === 'director')
    ...
    const normalizedOptions = options.length >= 2
      ? options
      : [
        'Option A: Ship a narrow vertical slice this cycle to validate signal quickly.',
        'Option B: Ship full initiative scope now with stronger verification gates.',
        'Option C: Defer scope and run architecture hardening before feature delivery.',
      ]
    const risks = Array.isArray(params.risks)
      ? params.risks.map((item) => String(item).trim()).filter(Boolean)
      : []
    const normalizedRisks = risks.length > 0
      ? risks
      : [
        'Risk: Merge conflicts across parallel worktrees. Mitigation: queue dry-run before merge.',
        'Risk: Incomplete human verification. Mitigation: explicit VERIFY gate before merge.',
        'Risk: Proposal ambiguity. Mitigation: include explicit yes/no decision question.',
      ]
    const constraints = Array.isArray(params.constraints)
      ? params.constraints.map((item) => String(item).trim()).filter(Boolean)
      : []
    const normalizedConstraints = constraints.length > 0
      ? constraints
      : [
        'Must preserve mission-control operability during rollout.',
        'Must keep merge gating: verified + conflict-free + clean target.',
        'Must produce explicit evidence in markdown records.',
      ]
    const evidence = [
      ...(discussion.thread.slice(0, 5).map((line) => line.replace(/^\d{4}-\d{2}-\d{2}T[^·]+·\s*/, '').trim())),
      ...discussion.outcomes.slice(0, 3),
    ].filter(Boolean)
    const decisionQuestion = ensureYesNoQuestion(
      normalizeQuestion(params.question) || `Should CEO approve the director proposal packet for "${initiative.title}"?`,
      initiative.title
    )
    const recommendation =
      (typeof params.recommendation === 'string' && params.recommendation.trim())
        ? params.recommendation.trim()
        : 'Recommend Option B: complete scope with strict verification and rollback-safe merge flow.'
    const summary =
      (typeof params.summary === 'string' && params.summary.trim())
        ? params.summary.trim()
        : (initiative.summary || `Deliver ${initiative.title} through proposal-approved execution.`)
    const actor = typeof params.actor === 'string' && params.actor.trim() ? params.actor.trim() : 'director'

    const proposal = upsertProposalRecord(brain.path, {
    ...
    return {
      message: `proposal_generated:${proposal.id}`,
      proposal: {
        id: proposal.id,
        filePath: proposal.rel,
        initiativeId: initiative.id,
        discussionId: discussion.id,
        decisionQuestion,
        recommendation,
  ...
  if (method === 'decision.record') {
    ...
    const decision = createRecord(brain.path, 'decisions', title, {
      ...
      proposal_id: typeof params.proposalId === 'string' ? params.proposalId : '',
      proposal_path: typeof params.proposalPath === 'string' ? params.proposalPath : '',
      discussion_id: typeof params.discussionId === 'string' ? params.discussionId : '',
      discussion_path: typeof params.discussionPath === 'string' ? params.discussionPath : '',
    })
    const event = lifecycleEvent(brainId, method, {
      ...params,
